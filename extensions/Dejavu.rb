module Dejavu
  ...
  def apply(operators, methods, predicates, state, tasks, goal_pos, goal_not, debug = false)
    ...
    knots.uniq!
    knots.each {|method,decomposition,index|
      name = method.first
  ...
  def visit(method, methods, knots, visited = {})
    if visited.include?(method) then true
      ...
      method.drop(2).each {|decomposition| decomposition.last.each_with_index {|task,index| knots << [method, decomposition, index] if visit(task.first, methods, knots, visited.dup)}}
      false
    end
  end
end