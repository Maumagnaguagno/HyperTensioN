module Patterns
  ...
  def match_patterns(swaps, dependencies, operators, predicates, debug)
    ...
    operators.each {|op|
      ...
      (precond_pos & effect_del).each {|pre|
        if pre2 = effect_add.assoc(pre[0])
    ...
    operators.each {|op|
      ...
      operators.each {|op2|
        ...
        neg = [] #precond_not.select {|pre| op2[5].assoc(pre[0])}
    ...
    swaps.each {|op,pre_constraints|
      namesub = (name = op[0]).tr(hyphen, underscore)
    ...
    operators.each {|op| graph << "\n  #{op[0].tr(hyphen, underscore)} [label=\"#{op[0]}(#{op[1].join(sep)})\" shape=box]"}
  ...
  def find_goal_methods(operators, methods, predicates, goal_pos, goal_not, debug)
    ...
    goal_pos_complex = goal_pos.reject {|goal,|
      operators.any? {|op|
        op[4].assoc(goal) and op[2].none? {|pre,| predicates[pre]} and ((precond_not = op[3].select {|pre,| predicates[pre]}).empty? or (precond_not.size == 1 and precond_not[0][0] == goal))
      }
    }
    goal_not_complex = goal_not.reject {|goal,|
      operators.any? {|op|
        op[5].assoc(goal) and op[3].none? {|pre,| predicates[pre]} and ((precond_pos = op[2].select {|pre,| predicates[pre]}).empty? or (precond_pos.size == 1 and precond_pos[0][0] == goal))
      }
    }
  ...
  def inject_method_dependencies(swaps, methods, predicates, debug)
    ...
    methods.each {|met|
      if met[0] =~ /^dependency_([\w-]+)_before_([\w-]+)_for_([\w-]+)$/
        ...
        sub = nil
        methods.each {|met2|
          if met2[0] =~ /^dependency_(?!#{dependent})[\w-]+_before_#{dependency}_for_([\w-]+)$/
            sub = met2
            break if $1 == pred
          end
        }
        ...
        # Prefer dependency with same predicate goal
        methods.each {|met2|
          if not met.equal?(met2) and met2[0] =~ /^dependency_swap_[\w-]+_until_[\w-]+_before_#{dependent}_for_([\w-]+)$/
            sub = met2
            break if $1 == pred
          end
        }
  ...
  def add_tasks(goal_methods, operators, methods, predicates, tasks, goal_pos, goal_not, debug)
    ...
    goal_pos.each {|goal|
      operators.each {|op|
        if group = op[4].assoc(goal[0])
          # TODO add unification method when required
          tasks_goals.unshift([op[1].map {|var| (i = group.index(var)) ? goal[i] : var}.unshift(op[0]), true, goal])
          break
        end
    ...
    goal_not.each {|goal|
      operators.each {|op|
        if group = op[5].assoc(goal[0])
          # TODO add unification method when required
          tasks_goals.unshift([op[1].map {|var| (i = group.index(var)) ? goal[i] : var}.unshift(op[0]), false, goal])
          break
        end
    ...
    Knoblock.create_hierarchy(operators, predicates).each {|order|
      order = [order] unless order[0].instance_of?(Array)
      order.each {|i| i[1] = i[1][0]}
      tasks_goals.reject! {|met,type,goal| ordered_tasks.unshift(met) if order.include?([type, goal[0]])}
  ...
  def compose_swap_methods(swaps, operators, methods, predicates, debug)
    ...
    swap_predicates.each {|(predicate_name,*predicate_terms),swap_ops|
      ...
      agent = predicate_terms[0] if predicate_terms.size != 1
      original_current = predicate_terms[-1]
      ...
      swap_ops.each {|op,constraints|
        original_intermediate = (constraints[0] - [original_current])[-1]
        ...
        new_op = op[1].map {|i| i == agent ? i : free_variables[i]}.unshift(op[0])
        ...
        effects.each {|eff|
          ...
          swap_method << ["using_#{op[0]}", free_variables,
  ...
  def compose_dependency_methods(swaps, dependencies, operators, methods, predicates, debug)
    ...
    dependencies.each_key {|op| disjunctions[[op[4], op[5]]] << op}.each {|op,op_dependencies|
      ...
      op_dependencies.each {|first,pos,neg|
        ...
        first_terms = first[1]
        swaps[first]&.each {|p,|
          if m = methods.assoc("swap_#{p[0]}_until_#{p[0]}")
            first = m
            if p[0] == pre[0]
        ...
        name = "dependency_#{first[0]}_before_#{seconds.map(&:first).join('_or_')}"
        ...
        if operators.include?(first)
          ...
          pre.zip(first[type ? 4 : 5].assoc(pre[0])) {|a,b|
            if a != b
              replacements[a] = b
              replacements[b] ||= a
            end
        ...
        seconds.each {|second|
          ...
          # Label and free variables
          satisfied << [seconds.size == 1 ? 'satisfied' : "satisfied_#{second[0]}", [],
            # Positive preconditions
            type ? precond_pos_second << pre : precond_pos_second,
            # Negative preconditions
            type ? precond_not_second : precond_not_second << pre,
            # Subtasks
            [[second[0], *second_terms]]
          ] unless first[0].start_with?(SWAP)
          # Label and free variables
          unsatisfied << [seconds.size == 1 ? 'unsatisfied' : "unsatisfied_#{second[0]}", [],
            # Positive preconditions
            ...
            # Subtasks
            [
              first_terms.map {|i| replacements[i] || i}.unshift(first[0]),
              [second[0], *second_terms]
            ]
          ]
        }
  ...
  def compose_unification_method(operators, methods, predicates, met, substitutions)
    ...
    unless methods.assoc(name = "unify#{free.join.tr!('?','_')}_before_#{met[0]}")
      # Find rigid predicates shared across decompositions to act as preconditions
      precond_pos = []
      precond_not = []
      met.drop(3).each {|dec|
        if dec[0].start_with?('unsatisfied', 'using_')
    ...
    free.each {|f|
      if precond_pos.none? {|pre| pre.include?(f)}
        ...
        while node = fringe.shift
          visited[node[0]] = true
          if operators.assoc(node[0])
            break if node[2].any? {|pre|
              if pre.include?(f) and not precond_pos.assoc(pre[0])
                precond_pos << pre
                new_free.concat(pre.drop(1).reject! {|i| ground_var.include?(i) or free.include?(i)})
              end
            }
          elsif node[0].start_with?(DEPENDENCY, SWAP)
            # TODO fix new multiple unsatisfied dependencies and multiple swap operators over same predicate
            node[-1][4].reverse_each {|i,| fringe.unshift(operators.assoc(i) || methods.assoc(i)) unless visited.include?(i)}
