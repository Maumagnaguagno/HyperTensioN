module Pullup
  ...
  def apply(operators, methods, predicates, state, tasks, goal_pos, goal_not, debug = false)
    ...
    operators.reject! {|op| impossible << op.first unless counter.include?(op.first) and op[2].all? {|pre,| predicates[pre] || state.include?(pre)}}
    ...
    methods.each {|decompositions|
      ...
      decompositions.select! {|_,free,precond_pos,precond_not,subtasks|
        ...
        precond_pos.each {|pre,*terms|
          if not predicates[pre] and not (terms & free).empty? and s = state[pre] and (s = s.select {|i| not i.zip(terms) {|a,b| break true if a == b or b.start_with?('?')}}).size == 1
