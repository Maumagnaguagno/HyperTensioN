  ...
  def swap_methods(names, parameters, precond_pos, free_variables2 = parameters.sort.push('?ag', '?b'))
    ...
    methods = [
      ...
      ['swap_at_until_empty', ['?ag', '?to'],
        ['base', base_precond1.flatten(1).select! {|i| i.start_with?('?') and i != '?to'}.uniq, base_precond1, [], []]
      ],
      ['swap_empty_until_at', ['?from'],
        ['base', base_precond2.flatten(1).select! {|i| i.start_with?('?') and i != '?from'}.uniq, base_precond2, [], []],
      ],
